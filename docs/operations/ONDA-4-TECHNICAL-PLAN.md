...
## 7. Fechamento — status final por critério (atualizado em 2026-07-30, pós-merge)

| #   | Critério (§2)                                                    | Status final                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Drill de reconstrução de schema, verificado contra banco vivo    | **Concluído.** PR #15 mergeado (`main` em `574eee0`). Primeira execução manual em CI falhou (`db-schema-drill.yml`, run `30550387462`) — causa raiz: a imagem oficial do Postgres sobe um servidor temporário para `initdb` que fecha antes do servidor final subir, e `pg_isready` sozinho aceitava o servidor temporário como pronto. Corrigido em `scripts/db-drill/run.sh` (espera a segunda ocorrência de "database system is ready to accept connections" no log) — PR #16, mergeado (`main` em `2c6385a`). Revalidado com sucesso diretamente em `2c6385a` (run `30565878779`, "Banco pronto (tentativa 3/60, servidor final)") |
| 2   | RPO/RTO propostos e documentados                                 | **Concluído**, com correção material: a suposição original de que o plano Free tinha backup diário estava errada — confirmado (documentação oficial da Supabase) que o Free não tem nenhum backup automático. `RESILIENCE-RUNBOOK.md` §2 e §6 atualizados                                                                                                                                                                                                                                                                                                                                                                              |
| 3   | Monitoramento de disponibilidade ativo, com alerta funcional     | **Concluído.** `uptime-check.yml` validado ao vivo em `main` (run `30550394577`) — staging e produção OK, nenhum incidente aberto indevidamente                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 4   | Plano de resposta a incidente documentado                        | **Concluído** — `INCIDENT-RESPONSE-PLAN.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 5   | Runbook de backup/restore documentado                            | **Concluído** — `RESILIENCE-RUNBOOK.md`, incluindo a nota de decisão (§6) e a decisão final do Founder/PMO (§6.4): alternativa D, adiar restore real até o Gate R0                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 6   | Recomendação concreta para `can_admins_bypass`                   | **Investigação concluída; execução BLOQUEADA.** Autorização do Founder/PMO recebida para desligar `can_admins_bypass: false`. Confirmado contra a documentação oficial da API do GitHub que esse campo **não é aceito** pelo corpo de `PUT /repos/{owner}/{repo}/environments/{environment_name}` — não existe caminho de API/CLI para essa mudança, só a UI web com um clique autenticado do Founder. Nenhuma tentativa de escrita foi feita. **Único bloqueio humano restante da Onda 4** — ver `docs/security/GOVERNANCE-RECOMMENDATION-ADMIN-BYPASS.md`                                                                            |
| 7   | `lint && test && build` zero; CI/CodeQL verdes                   | **Concluído** em todos os PRs (#15, #16, #17)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 8   | Nenhum dado real, produção, DNS ou Environment alterado sem gate | **Confirmado.** Nenhum deploy disparado nesta Onda; DNS de `vipreco.com.br` sem A/AAAA; produção seguiu vazia; nenhum secret exposto; nenhuma configuração de Environment alterada — o snapshot capturado antes da tentativa de mudança (`GOVERNANCE-RECOMMENDATION-ADMIN-BYPASS.md` §1) permanece idêntico ao estado atual                                                                                                                                                                                                                                                                                                            |

**Status da Onda 4: implementação e documentação concluídas; falta exclusivamente uma
ação humana fora do alcance de API/CLI (desmarcar `can_admins_bypass` na UI do GitHub).
Onda 4 não deve ser marcada `✅ Concluída` em `PLANO-MESTRE.md` até essa ação acontecer
e ser verificada — decisão do PMO/Founder, não deste CTO.**
