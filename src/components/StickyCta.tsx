...
import type { StickyCtaStore } from "@/lib/cta-visibility";
 ...
 * um marcado com o atributo que este componente recebe. Nunca dois botões idênticos disputando a
 * mesma tela. E enquanto ele está no ar, o CTA equivalente sai da ordem de foco e da árvore
 * acessível: fora da tela não é o mesmo que fora do caminho de quem navega por teclado ou leitor
 * de tela.
 *
 ...
 *
 * É o mecanismo, não o convite: o rótulo, o destino, o marcador e a loja de visibilidade vêm de
 * quem usa. A Home e `/para-mercados` compartilham a regra, e nada além dela — cada rota tem a
 * sua loja, e uma nunca silencia o CTA da outra.
 */
...
interface StickyCtaProps {
  /** Destino já resolvido. Sem destino, nada é renderizado — nem botão, nem espaçador. */
  href: string | null;
  /** Atributo que marca, no DOM, os CTAs equivalentes do fluxo desta página. */
  marcador: string;
  /** Loja de visibilidade desta família de CTA. Ver `@/lib/cta-visibility`. */
  loja: StickyCtaStore;
...
export function StickyCta({
  ...
  useEffect(() => {
    ...
    const equivalentes = Array.from(document.querySelectorAll(`[${marcador}]`));
